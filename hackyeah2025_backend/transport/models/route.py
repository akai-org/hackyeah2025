...
class Route(models.Model):
    ...
    line_number = models.CharField(
        max_length=50, verbose_name="Line number", help_text="e.g. IC 1234, Bus 125"
    )
    carrier = models.ForeignKey(
        Carrier, on_delete=models.CASCADE, related_name="routes", verbose_name="Carrier"
    )
...
class RoutePoint(models.Model):
    ...
    def get_track_conflicts(self):
        ...
        if self.track:
            return (
                RoutePoint.objects.filter(track=self.track)
                .exclude(id=self.id)
...
class RouteGraph(models.Model):
    ...
    def get_next_connection(self):
        ...
        return RouteGraph.objects.filter(
            route=self.route, sequence=self.sequence + 1
        ).first()
    ...
    def get_all_stations_on_route(self):
        ...
        route_connections = (
            RouteGraph.objects.filter(route=self.route)
            .order_by("sequence")
            .select_related("connection__from_station", "connection__to_station")
        )
